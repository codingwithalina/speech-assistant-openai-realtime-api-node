...
fastify.all("/incoming-call", async (request, reply) => {
  const twimlResponse = `<?xml version="1.0" encoding="UTF-8"?>
                          <Response>
                              <Say voice="Google.de-DE-Chirp3-HD-Aoede">Please wait while we connect your call to the A. I. voice assistant, powered by Twilio and the Open A I Realtime API</Say>
                              <Pause length="1"/>
                              <Say voice="Google.de-DE-Chirp3-HD-Aoede">O.K. you can start talking!</Say>
                              <Connect>
...
fastify.register(async (fastify) => {
  fastify.get("/media-stream", { websocket: true }, (connection, req) => {
    ...
    openAiWs.on("open", () => {
      ...
      // Sanity-Check: Assistant spricht einmal kurz zuerst
      setTimeout(() => {
        try {
          openAiWs.send(
            JSON.stringify({
              ...
              item: {
                ...
                content: [
                  {
                    type: "input_text",
                    text: "Say a short greeting for an audio test.",
                  },
                ],
              },
            })
